...
def greeting(season: SeasonDTO | None, *, app: bool = False) -> str:
    """«О клубе»: in the bot the report is sent right here, in the app — on the «Неделя» tab."""
    current = escape(season.title) if season else "пока не выбрана"
    how = (
        "Чтобы сдать — отправь текст или фото во вкладке «Неделя». Больше ничего нажимать не надо."
        if app
...
GREETING_CTA = (
    ...
    "«Рюкзак» с паспортом, журналом и твоими словами, «Карта» со словами недель."
)
...
"""Answer to the door button when the keyboard could not open the app itself."""
...
_HELP_ITEMS: tuple[tuple[str, str, str | None], ...] = (
    (
        ...
        "в «Рюкзаке», в журнале, пока неделя идёт.",
    ),
    (
        ...
        "Просто отправь ещё один отчёт: повторный штамп не понижает, а фото за текущую неделю поднимет его до "
    ...
    (
        ...
        "Можно, до конца сезона — в приложении: «🎒 Открыть клуб», «Карта», нажми на неделю, "
        ...
        "Можно, до конца сезона: вкладка «Карта», нажми на неделю, там «Добавить в журнал». "
    ...
    (
        ...
        "не видит. Нажми «✉️ Написать Миле» под этим сообщением и скажи, я поставлю.",
        "Скорее всего я просто не заметила — комментарии и встречи бот "
        "не видит. В «Рюкзаке» у заморозок есть «как заработать ещё?» — там можно мне написать. Я поставлю.",
    ),
    (
        "Что-то другое",
...
ADMIN_MEMO = (
    ...
    "Неделя — день, задание со словом, «Берусь · В этот раз мимо», форма отчёта\n"
    "Рюкзак — паспорт со штампами, заморозки, ачивки, свои слова и факты, журнал и PDF\n"
    "Карта — о клубе, недели летописью, слова недель, факты\n"
...
FACT_PROMPT_ADMIN = (
    ...
    "его увидят все на «Карте», и он попадёт в журналы всех.</i>"
)
FACT_SAVED = "Спасибо, записала ✅ Факт останется у тебя — и попадёт в твой журнал сезона."
MAX_FREEZE_BONUS = (
    "\n\n❄️ И тебе +1 заморозка за первый максимум — это право пропустить неделю так, чтобы цепочка не порвалась."
)
FACT_FREEZE_BONUS = (
    "\n\n❄️ И тебе +1 заморозка — это право пропустить неделю так, чтобы цепочка не порвалась. "
    "Все заморозки — в «Рюкзаке»."
...
def today_text(
    ...
    *,
    ...
    lines.append("Задание недели — во вкладке «Неделя», за кнопкой «🎒 Открыть клуб».")
    return "\n".join(lines)
...
INTENT_WEEK_OVER = "Эта неделя уже прошла — дописать за неё можно в приложении, на «Карте»."
...
def freeze_given(reason: str) -> str:
    return (
        ...
        "Это право пропустить неделю так, чтобы цепочка не порвалась. "
        "Все заморозки — в «Рюкзаке», за кнопкой «🎒 Открыть клуб»."
    )
...
def reminder_sunday(week: WeekDTO) -> str:
    return (
        ...
        "Даже минимум на пять минут считается."
    )
