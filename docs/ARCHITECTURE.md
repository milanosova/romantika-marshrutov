...
| `weeks` | `season_id FK`, `number int`, `title`, `starts_on`, `ends_on`, `intro`, `task_min`, `task_max`, `word`, `word_ru`, `word_meaning`, `announced_at timestamptz NULL` (NULL = draft; set once by `announce_week`, never cleared) | unique(season_id, number); weeks may not overlap inside a season; participant readers filter `announced_at IS NOT NULL` |
...
- API (JSON, all under `/api`, Pydantic schemas in `romantika/web/schemas.py`):
  ...
  admin: `GET/PUT /api/admin/seasons/{id}`, `GET /api/admin/season`, `GET/POST /api/admin/weeks`, `PUT/DELETE /api/admin/weeks/{id}`, `PATCH /api/admin/weeks/{id}/calendar`, `POST /api/admin/weeks/{id}/announce`,
...
- Admin API: `GET /api/admin/season` (the active season's dates), `GET /api/admin/weeks` (all weeks of the active season, drafts included, calendar order), `POST /api/admin/weeks` body `{number, starts_on, ends_on, title?, …}` → 201 (409 overlap / taken number / outside the season / not in the future, 422 malformed), `PUT /api/admin/weeks/{id}`
  body = subset of editable fields (422 for anything else), `PATCH /api/admin/weeks/{id}/calendar` body `{number?, starts_on?, ends_on?}` → 200 (409 started week or calendar conflict, 404 not in the active season), `POST /api/admin/weeks/{id}/announce` → 200 (422 without title/task_min, 409 week over), `DELETE /api/admin/weeks/{id}` → 204 (409 started or referenced, 404 not in the active season), `GET /api/admin/participants`
